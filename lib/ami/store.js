 ...
 * signature_hash = sha256(handle + signed_at + system_id + assessment_id)
 * Avoids circular dependency with integrity hash.
 */
function computeSignatureHash(handle, signedAt, systemId, assessmentId) {
  return crypto.createHash('sha256')
    .update(handle + signedAt + systemId + assessmentId, 'utf8')
    .digest('hex');
}
